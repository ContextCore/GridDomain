...
namespace GridDomain.EventSourcing.Adapters
{
    public class VersionedTypeSerializationBinder : SerializationBinder
    {
        ...
        public override Type BindToType(string assemblyName, string typeName)
        {
            ...
            if (originalType == null) throw new CantFindTypeException(originalTypeFullName);

            var typeMaxVersion = 0;
            if (!_typeMaxVersions.TryGetValue(originalType, out typeMaxVersion)) throw new CantFindTypeLatestVersionException(originalType);

            //if max version is reached, should return type without version number
            //because it is most recent version used in code now
            if (typeMaxVersion == versionedTypeName.Version) return originalType;
