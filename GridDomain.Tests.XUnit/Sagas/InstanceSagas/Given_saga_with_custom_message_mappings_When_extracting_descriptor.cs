    ...
    public class Given_saga_with_custom_message_mappings_When_extracting_descriptor
    {
      ...
      [Fact]
        public void Descriptor_contains_all_command_types_from_saga()
        {
            ...
            Assert.Equal(expectedCommands, _descriptor.ProduceCommands);
        }

      [Fact]
        public void Descriptor_contains_message_start_saga()
        {
            Assert.Equal(new[] { typeof(GotTiredEvent), typeof(SleptWellEvent) }, _descriptor.StartMessages);
        }

      [Fact]
        public void Descriptor_contains_saga_type()
        {
            Assert.Equal(typeof(ISagaInstance<CustomRoutesSoftwareProgrammingSaga, SoftwareProgrammingSagaData>), _descriptor.SagaType);
        }
      ...
      [Fact]
        public void Descriptor_contains_all_domain_event_types_from_saga()
        {
            ...
            Assert.Equal(_descriptor.AcceptMessages.Select(m => m.MessageType), expectedEvents);
        }
