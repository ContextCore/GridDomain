...
namespace GridDomain.Tests.XUnit
{
    public static class TestKitExtensions
    {
        ...
        public static async Task<SagaStateAggregate<TSagaState>> LoadSaga<TSaga, TSagaState>(this TestKit kit, Guid id) where TSagaState : class, ISagaState
            where TSaga : Saga<TSagaState>
        {
            var name = AggregateActorName.New<SagaStateAggregate<TSagaState>>(id).ToString();
            var actor = await kit.LoadActor<SagaActor<ISagaInstance<TSaga, TSagaState>, SagaStateAggregate<TSagaState>>>(name);
