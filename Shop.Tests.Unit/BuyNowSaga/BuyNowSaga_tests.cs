...
namespace Shop.Tests.Unit.BuyNowSaga
{
    ...
    internal class BuyNowSaga_tests
    {
        ...
        [Test]
        public void
            Given_reserving_state_When_order_total_calculated_Then_reserve_stock_command_is_issued_And_state_is_not_changed()
        {
            var scenario = NewScenario();
        ...
        public void Given_sku_purchase_ordered_Then_buy_now_saga_is_created_and_create_order_command_issued()
        {
            ...
            Assert.AreEqual(sagaId, scenario.Saga.State.Id);
        }

        [Test]
        public void
            Given_taking_stock_state_When_stockReserveTaken_Then_complete_order_and_complete_user_pending_order_commands_are_issued
            ()
        {
            var scenario = NewScenario();
            var sagaId = Guid.NewGuid();
