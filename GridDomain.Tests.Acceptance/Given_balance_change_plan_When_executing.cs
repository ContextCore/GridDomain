...
namespace GridDomain.Tests.Acceptance
{
    ...
    public class Given_balance_change_plan_When_executing : NodeCommandsTest
    {
        ...
        private void Then_balance_amounts_should_be_equal_to_plans(
            IReadOnlyCollection<BalanceChangePlan> balanceManipulationPlans)
        {
            using (var context = new BusinessBalanceContext())
            {
                ...
                {
                    Console.WriteLine(
                        $"Checking balance id {tData.balanceId}, expecting amount: {tData.TotalAmountChange}");
            ...
            IReadOnlyCollection<BalanceChangePlan> balanceManipulationCommands)
        {
            ...
            {
                ExecuteAndWaitFor<BalanceCreatedProjectedNotification>(cmd, new[] {cmd.BalanceId},
