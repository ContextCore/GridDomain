...
namespace GridDomain.Tests.Acceptance.Persistence
{
    ...
    public class DataGenerator
    {
        ...
        {
            return Enumerable.Range(0, businessNum).Select(bId => CreateBalanceChangePlan(Guid.NewGuid(), Guid.NewGuid(), commandsNum)).ToArray();
        }
        ...
        private BalanceChangePlan CreateBalanceChangePlan(Guid businessId,Guid balanceId, int commandsNum)
        {
            ...
            generator.Customizations.Add(new KnownConstructorParameter<ReplenishBalanceCommand,Guid>("balanceId", balanceId));
            generator.Customizations.Add(new KnownConstructorParameter<WithdrawalBalanceCommand,Guid>("balanceId", balanceId));
            generator.Customizations.Add(new KnownConstructorParameter<CreateBalanceCommand,Guid>("balanceId", balanceId));
            generator.Customizations.Add(new KnownConstructorParameter<CreateBalanceCommand,Guid>("businessId", businessId));

            var rnd = new Random();
            int numOfReplenishCommands = rnd.Next(1, commandsNum);
            int numOfWithdrawalCommands = commandsNum - numOfReplenishCommands;
