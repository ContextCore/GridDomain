...
namespace GridDomain.Tests.Acceptance.Scheduling.TestHelpers
{
                                             ...
                                             IProcessManagerCreator<TestProcessState>
    {
        ...
        public IProcessInstance<TestProcessState> CreateNew(Guid message, Guid? processId = null)
        {
            return CreateNew(new TestProcessStartMessage(processId ?? Guid.NewGuid(), null, processId ?? message));
        }

        public IProcessInstance<TestProcessState> Create(TestProcessState message)
        {
            return new ProcessManager<TestProcessState>(new TestProcess(), message, _log);
        }

        public IProcessInstance<TestProcessState> CreateNew(TestProcessStartMessage message, Guid? processId = null)
        {
