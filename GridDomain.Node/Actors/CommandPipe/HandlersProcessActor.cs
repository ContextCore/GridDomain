    ...
    public class HandlersProcessActor : ReceiveActor
    {
        ...
        public HandlersProcessActor(IProcessorListCatalog handlersCatalog, IActorRef sagasProcessActor)
        {
            ...
            Receive<IMessageMetadataEnvelop<IFault>>(envelop =>
                                                     {
                                                         SynhronizeHandlers(envelop)
                                                             .ContinueWith(
                                                                 t =>
                                                                     new AllHandlersCompleted(envelop.Metadata,
                                                                         envelop.Message))
                                                             .PipeTo(Self, Sender);
                                                         ;
                                                     });
            ...
            Receive<AllHandlersCompleted>(m =>
                                          {
                                              ...
                                              if (m.DomainEvents?.Length > 0)
                                                  _sagasProcessActor.Tell(
                                                      new MessageMetadataEnvelop<DomainEvent[]>(m.DomainEvents, m.Metadata));

                                              if (m.Fault != null)
                                                  _sagasProcessActor.Tell(new MessageMetadataEnvelop<IFault>(m.Fault,
                                                      m.Metadata));
                                          });
        }
